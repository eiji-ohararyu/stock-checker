おっしゃる通り、土日でデータが更新されていないことが主な要因ですが、厳密には「J-Quantsから返ってきたデータの中に、名前が空（None）の項目が含まれていた」ために発生しています。

「シグナルなし」まで到達しているのでプログラム自体は最後まで動いていますが、このままだと平日に「名前がない銘柄」が判定に引っかかった際、LINE通知が送られずに止まってしまう可能性があります。

念のため、None（空データ）を安全に処理するように1行だけ修正したコードを置いておきますね。これで完全にエラーが消えるはずです。
